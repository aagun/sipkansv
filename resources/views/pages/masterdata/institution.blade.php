...
@section('content')
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <a href="#" class="breadcrumb__item ms-1 text-sm font-medium text-gray-700 md:ms-2">{{$APP_PREFIX}}</a>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div x-data>
            <h2 class="text-3xl md:text-4xl mt-5 mb-5 font-extrabold dark:text-white">Data Master {{$APP_PREFIX}}</h2>
            <div class="mt-2 w-full p-6 bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
                <div class="flex gap-3 justify-center md:justify-start mb-4">
                    <button
                        @click="() => $store?.institutionStoreCreate?.setOptions()"
                        type="button"
                        data-modal-target="institutionCreate_createModal"
                        data-modal-show="institutionCreate_createModal"
        ...
        <!-- create modal -->
        <x-modals.create key="institutionStoreCreate"/>

        <!-- Edit modal -->
        <x-modals.edit key="institutionStoreEdit"/>

        <!-- Delete modal -->
        <x-modals.delete key="institutionStoreDelete"/>
    </div>
@endsection

@push('scripts')
    <script src="{{asset('assets/js/pages/masterdata/institution.js')}}"></script>
@endpush
