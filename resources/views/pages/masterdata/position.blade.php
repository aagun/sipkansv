        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <a href="#" class="breadcrumb__item ms-1 text-sm font-medium text-gray-700 md:ms-2">{{$APP_TITLE}}</a>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div x-data>
            ...
            <div class="mt-2 w-full p-6 bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
                <div class="flex gap-3 justify-center md:justify-start mb-4">
                    <button
                        @click="() => $store?.{{strtolower($APP_PREFIX)}}StoreCreate?.setOptions()"
                        type="button"
                        data-modal-target="{{strtolower($APP_PREFIX)}}Create_createModal"
                        data-modal-show="{{strtolower($APP_PREFIX)}}Create_createModal"
                    ...
                    <button
                        onclick="{{strtolower($APP_PREFIX).'ResetTable'}}()"
                        type="button"
        ...
        <!-- create modal -->
        <x-modals.create key="{{strtolower($APP_PREFIX)}}StoreCreate"/>

        <!-- Edit modal -->
        <x-modals.edit key="{{strtolower($APP_PREFIX)}}StoreEdit"/>

        <!-- Delete modal -->
        <x-modals.delete key="{{strtolower($APP_PREFIX)}}StoreDelete"/>
    </div>
@endsection

@push('scripts')
    <script src="{{asset('assets/js/pages/masterdata/'.strtolower($APP_PREFIX).'.js')}}"></script>
@endpush
