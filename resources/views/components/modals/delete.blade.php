...
<div :id="$store.{{$key}}.prefix + '_deleteModal'" tabindex="-1" aria-hidden="true"
     class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
    <div class="relative p-4 w-full max-w-md h-full md:h-auto">
        <!-- Modal content -->
        <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            <button
                :data-modal-toggle="$store.{{$key}}.prefix + '_deleteModal'"
                type="button"
            ...
            <p class="mb-4 text-gray-500 dark:text-gray-300" x-html="$store.{{$key}}.options?.modalTitle">Apakah anda yakin ingin menghapus data?</p>
            <div class="flex justify-center items-center space-x-4">
                <button
                    type="button"
                    @click="() => $store.{{$key}}.methods.delete();"
                ...
                <button
                    type="button"
                    :data-modal-toggle="$store.{{$key}}.prefix + '_deleteModal'"
                    class="w-full md:w-fit px-3 py-2.5 text-sm font-medium border-1 border-gray-500 text-gray-500 flex gap-2 justify-center md:inline-flex md:items-center bg-white-500 hover:bg-white-600 focus:ring-4 focus:outline-none focus:ring-gray-300 rounded-lg text-center dark:bg-gray-600 dark:hover:bg-gray-700 dark:focus:ring-gray-800">
